...
PRODUCTS = [
    ...
    ("BTL-500-24", "Packaged Drinking Water 500ml (Pack of 24)", "bottle", 0.5, 24, 240, 288,
     "\U0001f4a7", f"{IMG}/bottle-500ml.jpg", "instant", 35, 180,
     "Sealed 500ml bottles, ideal for events, offices and travel. BIS certified."),
    ("BTL-1L-12", "Packaged Drinking Water 1L (Pack of 12)", "bottle", 1.0, 12, 216, 240,
     "\U0001f6b0", f"{IMG}/bottle-1l.jpg", "instant", 35, 150,
     "Everyday 1 litre bottles in a carry-friendly pack of 12."),
    ("BTL-2L-9", "Packaged Drinking Water 2L (Pack of 9)", "bottle", 2.0, 9, 288, 324,
     "\U0001f9f4", f"{IMG}/bottle-2l.jpg", "same_day", 60, 120,
     "Family-size 2 litre bottles. Best value per litre in the bottle range."),
    ("BTL-CASE-48", "Bulk Bottle Case 500ml (Pack of 48)", "bottle", 0.5, 48, 450, 560,
     "\U0001f4e6", f"{IMG}/bottle-pack.jpg", "same_day", 75, 80,
     "Double case for functions, site offices and long meetings."),

    ("CAN-20L-1", "20L Mineral Water Can (Single)", "can", 20.0, 1, 60, 70,
     "\U0001faa3", f"{IMG}/can-20l.jpg", "instant", 40, 300,
     "The daily-use 20 litre jar. RO purified and UV treated. Empty can exchanged on delivery."),
    ("CAN-20L-3", "20L Mineral Water Can (Pack of 3)", "can", 20.0, 3, 165, 210,
     "\U0001f4e6", f"{IMG}/can-20l-alt.jpg", "instant", 45, 200,
     "Three 20L jars at a bundle rate. Most popular with families."),
    ("CAN-20L-10", "20L Mineral Water Can (Pack of 10)", "can", 20.0, 10, 520, 700,
     "\U0001f3ed", f"{IMG}/can-bulk.jpg", "same_day", 90, 90,
     "Bulk pack of ten 20L jars for offices, hostels and PGs."),
    ("CAN-PUMP-1", "20L Can with Manual Pump", "can", 20.0, 1, 145, 190,
     "⛽", f"{IMG}/can-pump.jpg", "instant", 45, 110,
     "A 20L jar bundled with a reusable hand pump. No dispenser needed."),

    ("CMP-50L", "50L Water Camper with Tap", "camper", 50.0, 1, 320, 380,
     "\U0001f376", f"{IMG}/camper-50l.jpg", "same_day", 90, 60,
     "Insulated 50 litre camper with a dispensing tap. Great for small gatherings."),
    ("CMP-100L", "100L Party Camper (Chilled)", "camper", 100.0, 1, 580, 680,
     "\U0001f389", f"{IMG}/camper-party.jpg", "scheduled", 150, 40,
     "Chilled 100 litre camper for parties and functions. Stand included."),
    ("CMP-200L", "200L Event Camper (Chilled + Stand)", "camper", 200.0, 1, 1050, 1250,
     "\U0001f3aa", f"{IMG}/camper-pour.jpg", "scheduled", 180, 25,
     "Large 200 litre chilled camper for weddings and large events."),
    ("CMP-OFC-50", "Office Dispenser Camper 50L (Hot & Cold)", "camper", 50.0, 1, 420, 500,
     "\U0001f3e2", f"{IMG}/camper-office.jpg", "scheduled", 120, 35,
     "Hot-and-cold dispenser camper on a monthly-friendly rate. Popular with offices."),
]

TANKER_TIERS = [
    ("TNK-IND-1000", 1000, "individual", 450, 90, f"{IMG}/tanker-truck.jpg",
     "Compact tanker for a single home, a small top-up or garden use."),
    ("TNK-IND-2000", 2000, "individual", 750, 120, f"{IMG}/tanker-rural.jpg",
     "Standard household tanker. Covers a typical family for 3 to 4 days."),
    ("TNK-IND-5000", 5000, "individual", 1450, 150, f"{IMG}/tanker-yellow.jpg",
     "Large household or small-function tanker."),
    ("TNK-SOC-5000", 5000, "society", 1350, 150, f"{IMG}/tanker-yellow.jpg",
     "Society rate for a 5000L trip. Best for small apartment blocks."),
    ("TNK-SOC-8000", 8000, "society", 1980, 180, f"{IMG}/tanker-street.jpg",
     "Bulk 8000L trip for societies, schools and institutions."),
    ("TNK-SOC-12000", 12000, "society", 2850, 210, f"{IMG}/tanker-truck.jpg",
...
VENDORS = [
    ...
    ("Indore Aqua Care", "9822001133", 0.021, -0.018, 4.8, 210, 486, True, KYC_APPROVED,
     1000, 8000, 1200, "3,4,7", "Round-the-clock tankers and cans across west Indore.",
    ...
    ("Shree Ganesh Tankers", "9827114520", -0.032, 0.024, 4.7, 168, 392, True, KYC_APPROVED,
     2000, 12000, 800, "1,2,5", "Bulk tanker specialists for societies and sites.",
    ...
    ("RK Water Suppliers", "9893227741", 0.014, 0.036, 4.5, 96, 214, True, KYC_APPROVED,
     1000, 5000, 550, "4,6", "Affordable daily cans and mid-size tankers.",
    ...
    ("Narmada Jal Seva", "9977330012", -0.018, -0.041, 4.6, 143, 305, True, KYC_APPROVED,
     2000, 8000, 900, "2,3,8", "Narmada-line water, delivered on schedule.",
    ...
    ("Malwa Pure Water Co.", "9826558890", 0.045, 0.012, 4.4, 74, 158, True, KYC_APPROVED,
     1000, 5000, 700, "5,6,7", "Packaged water and campers for offices and events.",
    ...
    ("Sai Krupa Tanker Service", "9755440021", -0.052, 0.031, 4.2, 51, 97, True, KYC_APPROVED,
     2000, 12000, 780, "1,8,9", "Construction and society tankers at short notice.",
    ...
    ("Vaishnavi Water Point", "9770112233", 0.008, -0.052, 4.9, 38, 71, True, KYC_APPROVED,
     1000, 3000, 620, "4,5", "Small-batch, high-purity cans for homes.",
    ...
    ("Ujjwal Jal Suppliers", "9691004455", 0.061, -0.033, 4.0, 6, 9, False, KYC_PENDING,
     1000, 5000, 690, "9,10", "New operator awaiting verification.",
